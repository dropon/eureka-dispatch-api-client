# \AddressCategoriesAPI

All URIs are relative to *https://mylicense.dispatchapi.dispatch-rts.com*

Method | HTTP request | Description
------------- | ------------- | -------------
[**GetAddressCategories**](AddressCategoriesAPI.md#GetAddressCategories) | **Get** /v3/address-categories | Get address categories



## GetAddressCategories

> IPagedResourceListAddressCategoryDto GetAddressCategories(ctx).Code(code).Label(label).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
...
func main() {
	code := "code_example" // string | Address category code pattern (optional)
	label := "label_example" // string | Address category label pattern (optional)
	...
	resp, r, err := apiClient.AddressCategoriesAPI.GetAddressCategories(context.Background()).Code(code).Label(label).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AddressCategoriesAPI.GetAddressCategories``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetAddressCategories`: IPagedResourceListAddressCategoryDto
	fmt.Fprintf(os.Stdout, "Response from `AddressCategoriesAPI.GetAddressCategories`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetAddressCategoriesRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **code** | **string** | Address category code pattern | 
 **label** | **string** | Address category label pattern | 
...
[**IPagedResourceListAddressCategoryDto**](IPagedResourceListAddressCategoryDto.md)
