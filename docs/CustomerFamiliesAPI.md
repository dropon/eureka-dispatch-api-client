...
------------- | ------------- | -------------
[**GetCustomerFamilies**](CustomerFamiliesAPI.md#GetCustomerFamilies) | **Get** /v3/customer-families | Get customer families



## GetCustomerFamilies

> IPagedResourceListCustomerFamilyDto GetCustomerFamilies(ctx).Code(code).Label(label).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()

Get customer families
...
func main() {
	...
	resp, r, err := apiClient.CustomerFamiliesAPI.GetCustomerFamilies(context.Background()).Code(code).Label(label).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CustomerFamiliesAPI.GetCustomerFamilies``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetCustomerFamilies`: IPagedResourceListCustomerFamilyDto
	fmt.Fprintf(os.Stdout, "Response from `CustomerFamiliesAPI.GetCustomerFamilies`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetCustomerFamiliesRequest struct via the builder pattern
...
[**IPagedResourceListCustomerFamilyDto**](IPagedResourceListCustomerFamilyDto.md)
