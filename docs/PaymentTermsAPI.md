...
------------- | ------------- | -------------
[**GetPaymentTerms**](PaymentTermsAPI.md#GetPaymentTerms) | **Get** /v3/payment-terms | Get payment terms



## GetPaymentTerms

> IPagedResourceListPaymentTermDto GetPaymentTerms(ctx).Code(code).Label(label).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
...
func main() {
	code := "code_example" // string | Payment term code pattern (optional)
	label := "label_example" // string | Payment term label pattern (optional)
	...
	resp, r, err := apiClient.PaymentTermsAPI.GetPaymentTerms(context.Background()).Code(code).Label(label).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PaymentTermsAPI.GetPaymentTerms``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetPaymentTerms`: IPagedResourceListPaymentTermDto
...
------------- | ------------- | ------------- | -------------
 **code** | **string** | Payment term code pattern | 
 **label** | **string** | Payment term label pattern | 
...
[**IPagedResourceListPaymentTermDto**](IPagedResourceListPaymentTermDto.md)
