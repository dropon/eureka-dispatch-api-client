...
------------- | ------------- | -------------
[**GetAllSubServices**](SubServicesAPI.md#GetAllSubServices) | **Get** /v3/sub-services | Get sub services
...
> IPagedResourceListSubServiceDto GetAllSubServices(ctx).IsEnabled(isEnabled).Code(code).Label(label).Codes(codes).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
...
func main() {
	...
	resp, r, err := apiClient.SubServicesAPI.GetAllSubServices(context.Background()).IsEnabled(isEnabled).Code(code).Label(label).Codes(codes).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SubServicesAPI.GetAllSubServices``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetAllSubServices`: IPagedResourceListSubServiceDto
	fmt.Fprintf(os.Stdout, "Response from `SubServicesAPI.GetAllSubServices`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetAllSubServicesRequest struct via the builder pattern
