...
------------- | ------------- | -------------
[**GetUsers**](UsersAPI.md#GetUsers) | **Get** /v3/users | Get users



## GetUsers

> IPagedResourceListUserDto GetUsers(ctx).Name(name).IsDisabled(isDisabled).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
...
func main() {
	name := "name_example" // string | User name pattern (optional)
	isDisabled := true // bool |  (optional)
	...
	resp, r, err := apiClient.UsersAPI.GetUsers(context.Background()).Name(name).IsDisabled(isDisabled).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `UsersAPI.GetUsers``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetUsers`: IPagedResourceListUserDto
...
[**IPagedResourceListUserDto**](IPagedResourceListUserDto.md)
