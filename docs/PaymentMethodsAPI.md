...
------------- | ------------- | -------------
[**GetPaymentMethods**](PaymentMethodsAPI.md#GetPaymentMethods) | **Get** /v3/payment-methods | Get payment methods



## GetPaymentMethods

> IPagedResourceListPaymentMethodDto GetPaymentMethods(ctx).Code(code).Label(label).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
...
func main() {
	code := "code_example" // string | Payment method code pattern (optional)
	label := "label_example" // string | Payment method label pattern (optional)
	...
	resp, r, err := apiClient.PaymentMethodsAPI.GetPaymentMethods(context.Background()).Code(code).Label(label).Pattern(pattern).PatternFields(patternFields).StartIndex(startIndex).Count(count).Sort(sort).Desc(desc).Fields(fields).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PaymentMethodsAPI.GetPaymentMethods``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetPaymentMethods`: IPagedResourceListPaymentMethodDto
...
------------- | ------------- | ------------- | -------------
 **code** | **string** | Payment method code pattern | 
 **label** | **string** | Payment method label pattern | 
...
[**IPagedResourceListPaymentMethodDto**](IPagedResourceListPaymentMethodDto.md)
